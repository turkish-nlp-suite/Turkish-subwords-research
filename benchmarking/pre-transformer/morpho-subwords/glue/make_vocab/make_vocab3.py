...
def make_vocab(taskname):
  ...
  with open("valid_wcache_" + taskname + ".json", "w") as ofile:
    ofile.write(wcache)

  lenc = len(all_subwords)
  ordered_pairs = all_subwords.most_common(lenc)
  with open("valid_subwrd_counts_" + taskname + ".json", "w") as ofile:
