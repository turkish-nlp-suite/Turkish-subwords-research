...
class WordBiLSTMCRF(torch.nn.Module):
    def __init__(self, num_words, num_tags, emb_dim=300, hidden=256, num_layers=1, dropout=0.3, proj_dim=None):
        super().__init__()
        self.word_emb = torch.nn.Embedding(num_words, emb_dim, padding_idx=0)
        self.proj = None
        input_dim = emb_dim
...
def get_ft_vector(ft_obj, backend, token: str):
    if backend == "gensim":
        # gensim KeyedVectors supports `in` membership
        return ft_obj[token] if token in ft_obj else None
    else:
        # fasttext returns subword-backed vectors for OOV too
        try:
...
def init_embeddings_from_fasttext(model: WordBiLSTMCRF, word2id: Dict[str,int], id2word: List[str],
    ...
    if emb.embedding_dim != ft_dim and model.proj is None:
        # If user set emb_dim != FT dim, rely on projection; otherwise align model to FT dim first.
    ...
    with torch.no_grad():
        ...
        for i, tok in enumerate(id2word):
            if i < 2:  # skip PAD, UNK already set
                continue
    ...
    if verbose:
        vocab = len(id2word)
        print(f"[FastText] init done: vocab={vocab}, dim={ft_dim}, "
              f"frozen={freeze}, oov_policy={oov_policy}")
    ...
    for seed in args.seeds:
        ...
        if args.use_fasttext:
            if not args.fasttext_path:
                ...
                sys.exit(1)
            init_embeddings_from_fasttext(
                model, word2id, id2word,
                ft_path=args.fasttext_path,
                oov_policy=args.fasttext_oov,
                freeze=args.fasttext_lock_frozen,
            )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if dev_batches is not None:
                ...
                # No dev: track best by training loss (optional) and save last
