...
def load_vocab_file(path: str):
    with open(path, "r", encoding="utf-8") as f:
        itos = [line.rstrip("\n") for line in f]
    # Expect first two tokens are <pad>, <unk>. If not, prepend.
    if not (len(itos) >= 2 and itos[0] == PAD and itos[1] == UNK):
        itos = [PAD, UNK] + itos
...
# Subword helpers
# ===================
def fetch_subwords_from_cache(word: str, cache: Dict[str, List[str]], preserve_init_cap=True):
    if word is None or word == "":
        return [word]
    subwords = cache.get(word, [word])
    if preserve_init_cap and word[0].isupper() and len(subwords) > 0 and len(subwords[0]) > 0:
        fw = subwords[0]
        fw = fw[0].upper() + fw[1:]
        subwords = [fw] + subwords[1:]
    return subwords

def iob_expand_tags_for_subwords(tag: str, n_sub: int) -> List[str]:
    if n_sub <= 0:
        return []
    if tag == "O":
        return ["O"] * n_sub
    if "-" in tag:
        ...
        prefix, typ = "B", tag
    if prefix == "B":
        return ["B-" + typ] + ["I-" + typ] * (n_sub - 1)
    elif prefix == "I":
        return ["I-" + typ] * n_sub
    else:
        return ["I-" + typ] * n_sub

def words_to_subwords_and_tags(
    words: List[str],
    tags: List[str],
    cache: Dict[str, List[str]],
    subword2id: Dict[str, int],
    lower: bool = False,
    preserve_init_cap: bool = True,
) -> Tuple[List[int], List[str]]:
    sw_ids: List[int] = []
    sw_tags: List[str] = []
    for w, t in zip(words, tags):
        # Try exact form first to respect case-sensitive cache; fallback to normalized
        sw = cache.get(w, None)
        if sw is None:
            w_norm = normalize_token(w, lower=lower)
            sw = cache.get(w_norm, [w_norm])
        # If still not found, ensure capitalization policy applied consistently
        if sw is None or len(sw) == 0:
            sw = [w]
        if preserve_init_cap:
            sw = fetch_subwords_from_cache(w, cache, preserve_init_cap=True) if w in cache else sw

        ids = [subword2id.get(s, subword2id[UNK]) for s in sw]
        sw_ids.extend(ids)
        sw_tags.extend(iob_expand_tags_for_subwords(t, len(ids)))
    return sw_ids, sw_tags

# ===================
# Data prep
# ===================
def build_dataset_word(split_ds, word2id, id2label, label2id, text_col="tokens", label_col="tags",
...
def build_dataset_subword(split_ds, subword2id, id2label, label2id,
                          cache: Dict[str, List[str]],
                          text_col="tokens", label_col="tags",
                          lower=False):
    X, Y = [], []
    for ex in split_ds:
        words = [str(t) for t in ex[text_col]]
        tag_raw = ex[label_col]
        tags = [id2label[i] if isinstance(i, int) else str(i) for i in tag_raw]
        sw_ids, sw_tags = words_to_subwords_and_tags(
            words, tags, cache, subword2id, lower=lower, preserve_init_cap=True
        )
        tag_ids = tags_to_ids(sw_tags, label2id)
        X.append(sw_ids); Y.append(tag_ids)
    x_pad, mask = pad_batch(X, pad=subword2id[PAD])
    y_pad, _ = pad_batch(Y, pad=label2id["O"])
    return x_pad, y_pad, mask
...
def build_pretrained_weight_from_fasttext(id2word: List[str], ft_path: str, pad_token=PAD, unk_token=UNK,
    ...
    with torch.no_grad():
        # PAD
        try:
            pad_idx = id2word.index(pad_token)
            W[pad_idx] = 0.0
        except ValueError:
            pass
        # UNK
        try:
            unk_idx = id2word.index(unk_token)
            torch.nn.init.normal_(W[unk_idx], mean=0.0, std=0.02)
        except ValueError:
            pass
        # Fill others
...
def main():
    ...
    # Vocab handling
    ap.add_argument("--vocab_file", default=None, help="Path to (sub)word vocab.txt. First two lines should be <pad>, <unk>, otherwise they will be prepended.")
    ap.add_argument("--save_vocab", default=None)
    ap.add_argument("--max_vocab", type=int, default=50000)
    ap.add_argument("--min_word_freq", type=int, default=1)
    # Normalization
    ap.add_argument("--lower", action="store_true")
    ap.add_argument("--normalize_digits", action="store_true")  # retained for interface
    # FastText
    ...
    # Subword options
    ap.add_argument("--use_subwords", action="store_true", help="Expand words to subwords and align IOB tags.")
    args = ap.parse_args()
    ...
    assert "test" in raw, "Dataset must include a test split"
    ...
    # Choose vocab and dataset builder
    if args.use_subwords:
        assert args.vocab_file is not None, "--use_subwords requires --vocab_file pointing to subword list"
        subword2id, id2subword = load_vocab_file(args.vocab_file)
        with open("vocab_files/wcache_train.json", "r", encoding="utf-8") as f:
            train_cache = json.load(f)

        with open("vocab_files/wcache_test.json", "r", encoding="utf-8") as f:
            test_cache = json.load(f)

        with open("vocab_files/wcache_valid.json", "r", encoding="utf-8") as f:
            valid_cache = json.load(f)

        Xtr, Ytr, Mtr = build_dataset_subword(
            train_split, subword2id, id2label, label2id, train_cache,
            text_col=args.text_column, label_col=args.label_column, lower=args.lower
        )
        if dev_split is not None:
            Xdev, Ydev, Mdev = build_dataset_subword(
                dev_split, subword2id, id2label, label2id, valid_cache,
                text_col=args.text_column, label_col=args.label_column, lower=args.lower
            )
        Xte, Yte, Mte = build_dataset_subword(
            test_split, subword2id, id2label, label2id, test_cache,
            text_col=args.text_column, label_col=args.label_column, lower=args.lower
        )
        vocab2id, id2vocab = subword2id, id2subword
    else:
        if args.vocab_file:
        ...
        Xtr, Ytr, Mtr = build_dataset_word(train_split, word2id, id2label, label2id,
                                           text_col=args.text_column, label_col=args.label_column,
                                           lower=args.lower, normalize_digits=args.normalize_digits)
        if dev_split is not None:
            Xdev, Ydev, Mdev = build_dataset_word(dev_split, word2id, id2label, label2id,
                                                  text_col=args.text_column, label_col=args.label_column,
                                                  lower=args.lower, normalize_digits=args.normalize_digits)
        Xte, Yte, Mte = build_dataset_word(test_split, word2id, id2label, label2id,
                                           text_col=args.text_column, label_col=args.label_column,
                                           lower=args.lower, normalize_digits=args.normalize_digits)
        vocab2id, id2vocab = word2id, id2word
    ...
    for seed in args.seeds:
        ...
        if args.use_fasttext:
            ...
            pretrained_weight, ft_dim = build_pretrained_weight_from_fasttext(
                id2vocab, args.fasttext_path, oov_policy=args.fasttext_oov
            )
            effective_emb_dim = ft_dim
            freeze_emb = args.fasttext_lock_frozen

        model = WordBiLSTMCRF(
            num_words=len(vocab2id),
        ...
        torch.save(
            {"state_dict": model.state_dict(), "vocab2id": vocab2id, "label2id": label2id},
    ...
    with open(os.path.join(args.output_dir, "final.txt"), "w", encoding="utf-8") as of:
        ...
        of.write(f"LaTeX row: BiLSTM-CRF ({'subw' if args.use_subwords else 'word'}) & {mean_f1:.2f} ± {std_f1:.2f}\n")
