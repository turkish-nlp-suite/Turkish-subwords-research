...
def calculate_task_size(task, split, N):
  train_file = "{}/train_vocab.txt".format(task)
  ...
  counts_file = "{}/{}_word_counts_{}.txt".format(task, split, task)
  word_counts = {}
  ...
  subwrds_file = "{}/words_cache.json".format(task, split, task)
...
def calculate_all():
  ...
  tasks = ["e-commerce"]
  #sizes =  [50, 100, 200, 500, 800, 1000, 1500, 2000, 3000, 5000, 7000, "all"]
  sizes = [50, 100, 200, 500, 800, 1000, 1500, 2000, 3000, 5000, 10000, 15000, 20000, "all"]
  ...
  for task in tasks:
    ...
    for split in splits:
      print("SPLIT", split, "=========================")
      for size in sizes:
        calculate_task_size(task, split, size)
